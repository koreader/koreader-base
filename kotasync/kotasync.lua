...
local function make(tar_xz_path, kotasync_path, older_tar_xz_or_kotasync_path)
    local tar_xz = kotasync.TarXz:new(tar_xz_path)
...
local help = [[
USAGE: kotasync make [-h] [--reorder OLDER_TAR_XZ_OR_KOTASYNC_FILE] TAR_XZ_FILE [KOTASYNC_FILE]
...
local function main()
    ...
    if command == "make" then
        ...
        fn = function() make(arguments[1], arguments[2], options["reorder"]) end
