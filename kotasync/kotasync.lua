...
local function sync(state_dir, manifest_url, seed)
    ...
    local updater = kotasync.Updater:new(manifest_url, state_dir, seed)
    updater:fetch_manifest()
    local total_files = #updater.manifest.files
    local last_update = 0
    local delay = false --190000
    local update_frequency = 0.2
    local stats = updater:prepare_update(function(count, total)
    ...
    local old_progress
    last_update = 0
    local ok, err = pcall(updater.download_update, updater, function(count, total, index, path)
        local new_update = util.getTimestamp()
        if count ~= total and new_update - last_update < update_frequency then
            return true
        end
        last_update = new_update
        local padding = math.max(#prev_path, #path)
        local progress = math.floor(count / pbar_chunk)
        local pbar = pbar_indicators[#pbar_indicators]:rep(progress)..pbar_indicators[1 + math.floor(count % pbar_chunk * #pbar_indicators / pbar_chunk)]..(" "):rep(pbar_size - progress - 1)
        local new_progress = string.format("\rdownloading: %8s %4u/%4u %s %-"..padding.."s", count, index, stats.missing_files, pbar, path)
