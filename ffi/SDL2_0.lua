...
function S.waitForEvent(sec, usec)
    ...
    elseif event.type == SDL.SDL_MOUSEMOTION and event.motion.which ~= SDL_TOUCH_MOUSEID
        ...
        if is_finger then
            ...
            if pointers[0] and pointers[0].down then
                ...
                genTouchMoveEvent(event, slot, event.motion.x * S.w / S.win_w, event.motion.y * S.h / S.win_h)
            end
    ...
    elseif event.type == SDL.SDL_MOUSEBUTTONUP and event.button.which ~= SDL_TOUCH_MOUSEID
        ...
        local x = is_finger and event.tfinger.x * S.w or event.button.x * S.w / S.win_w
        local y = is_finger and event.tfinger.y * S.h or event.button.y * S.h / S.win_h
