...
function Updater:_recv()
    ...
    posix.read(self.read_fd, len, 2, true)
    posix.read(self.read_fd, self.buf:reset():reserve(len[0]), len[0], true)
    return unpack(self.buf:commit(len[0]):decode())
end

function Updater:_send(...)
    local len = ffi.new('uint16_t[1]', #self.buf:reset():encode{...})
    posix.write(self.write_fd, len, 2, true)
    posix.write(self.write_fd, self.buf:ref(), len[0], true)
    return self
end
...
function Updater:_call(method, progress_cb)
    ...
    if len > 0 then
        posix.read(self.read_fd, self.buf:reset():reserve(len), len, true)
    end
