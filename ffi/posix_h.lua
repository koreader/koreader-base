local ffi = require("ffi")

ffi.cdef[[
typedef unsigned int size_t;
typedef long int off_t;
struct timeval {
  long int tv_sec;
  long int tv_usec;
};
struct statvfs {
  long unsigned int f_bsize;
  long unsigned int f_frsize;
  long unsigned int f_blocks;
  long unsigned int f_bfree;
  long unsigned int f_bavail;
  long unsigned int f_files;
  long unsigned int f_ffree;
  long unsigned int f_favail;
  long unsigned int f_fsid;
  int __f_unused;
  long unsigned int f_flag;
  long unsigned int f_namemax;
  int __f_spare[6];
};
int pipe(int *) __attribute__((__nothrow__));
int fork(void) __attribute__((nothrow));
int dup(int) __attribute__((__nothrow__));
int dup2(int, int) __attribute__((__nothrow__));
static const int O_RDWR = 2;
static const int O_RDONLY = 0;
static const int O_NONBLOCK = 2048;
int open(const char *, int, ...);
int close(int);
int fcntl(int, int, ...);
int execl(const char *, const char *, ...) __attribute__((nothrow));
int execlp(const char *, const char *, ...) __attribute__((nothrow));
int execv(const char *, char *const *) __attribute__((nothrow));
int execvp(const char *, char *const *) __attribute__((nothrow));
int write(int, const void *, size_t);
int read(int, void *, size_t);
int kill(int, int) __attribute__((__nothrow__));
int waitpid(int, int *, int);
int getpid(void) __attribute__((__nothrow__));
int setpgid(int, int) __attribute__((__nothrow__));
struct pollfd {
  int fd;
  short int events;
  short int revents;
};
static const int POLLIN = 1;
static const int POLLOUT = 4;
static const int POLLERR = 8;
static const int POLLHUP = 16;
int poll(struct pollfd *, long unsigned int, int);
static const int PROT_READ = 1;
static const int PROT_WRITE = 2;
static const int MAP_SHARED = 1;
static const int MAP_FAILED = 4294967295;
static const int PATH_MAX = 4096;
int memcmp(const void *, const void *, size_t) __attribute__((pure, leaf, nothrow));
void *mmap(void *, size_t, int, int, int, long int) __attribute__((__nothrow__));
int munmap(void *, size_t) __attribute__((__nothrow__));
int ioctl(int, long unsigned int, ...) __attribute__((__nothrow__));
unsigned int sleep(unsigned int);
int usleep(unsigned int);
int statvfs(const char *restrict, struct statvfs *restrict) __attribute__((__nothrow__));
int gettimeofday(struct timeval *restrict, struct timezone *restrict) __attribute__((__nothrow__));
char *realpath(const char *restrict, char *restrict) __attribute__((__nothrow__));
char *basename(char *) __asm__("__xpg_basename") __attribute__((__nothrow__));
char *dirname(char *) __attribute__((__nothrow__));
void *malloc(size_t) __attribute__((malloc, leaf, nothrow));
void free(void *) __attribute__((leaf, nothrow));
char *strdup(const char *) __attribute__((malloc, leaf, nothrow));
char *strndup(const char *, size_t) __attribute__((malloc, leaf, nothrow));
struct _IO_FILE *fopen(const char *restrict, const char *restrict);
int fclose(struct _IO_FILE *);
int printf(const char *, ...);
int sprintf(char *, const char *, ...) __attribute__((nothrow));
int fprintf(struct _IO_FILE *restrict, const char *restrict, ...);
int fputc(int, struct _IO_FILE *);
static const int FIONREAD = 21531;
int fileno(struct _IO_FILE *) __attribute__((__nothrow__));
char *strerror(int) __attribute__((__nothrow__));
]]
