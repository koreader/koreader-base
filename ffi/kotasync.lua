...
function TarXz:open(filename, manifest)
    ...
    posix.read(fd, index_buf, footer_stream_flags.backward_size, true)
...
function Updater:fetch_manifest()
    ...
    if self.dl.status_code ~= 304 then -- 304: Not Modified.
        ...
        -- Update ETag file.
        if self.dl.etag and self.dl.etag:sub(1, 2) ~= "W/" then
...
function Updater:download_update(progress_cb)
    ...
    if C.HAVE_POSIX_FALLOCATE ~= 0 then
