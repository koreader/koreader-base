...
static int getTextFromPositions(lua_State *L) {
	...
	if (!startp.isNull() && !endp.isNull()) {
		...
		// feels clumsy but has the effect that it doesn't need to know and care about whether the
		...
		// But it is less OK with an initial long-press on a glyph, where we get 50% chances of
